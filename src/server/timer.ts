import { bookDict, barcodeQueue, eventQueue, enqueueEvent, type BookItem } from "./state";
import { emitSocket, getBeltSpeed } from "./runtime";
import { writeBucket } from "./plc";
import { requestPurescan } from "./purescan";
...
async function handleEvent(event: { type: string; payload: unknown; ts?: number }, now: number): Promise<void> {
  ...
  if (eventType === "photo_eye") {
    ...
    let emitData: BookItem | null = null;
    if (barcodeQueue.length > 0) {
      const item = barcodeQueue.shift();
      barcode = item?.barcode ?? null;
    }
    if (barcode && bookDict.has(barcode)) {
      const entry = bookDict.get(barcode)!;
  ...
  if (eventType === "purescan_ok") {
    ...
    if (response == null || typeof response !== "object") {
      let emitData: BookItem | null = null;
      if (bookDict.has(barcode)) {
        const b = bookDict.get(barcode)!;
...
async function onInterval100ms(): Promise<void> {
  ...
  for (const barcode of [...bookDict.keys()]) {
    const item = bookDict.get(barcode);
    if (!item) {
      continue;
    }
